...
```bash
TRANSITION=$(node "E:/Programacion/qn-arq-app/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "E:/Programacion/qn-arq-app/.claude/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "E:/Programacion/qn-arq-app/.claude/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "E:/Programacion/qn-arq-app/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
