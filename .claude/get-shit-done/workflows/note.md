...
- **Global scope**: `E:/Programacion/qn-arq-app/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/Programacion/qn-arq-app/.claude/notes/`)
...
2. Glob `E:/Programacion/qn-arq-app/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/Programacion/qn-arq-app/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/Programacion/qn-arq-app/.claude/notes/` — works in any directory
